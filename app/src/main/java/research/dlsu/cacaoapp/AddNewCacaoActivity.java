...
public class AddNewCacaoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_new_cacao);
        ...
        buttonSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Cacao crab = submitEntryIntoTheDatabase();
                if(crab!=null) {
    ...
    public void showProgressDialog(){
        progressDialog = ProgressDialog.show(AddNewCacaoActivity.this, "Uploading",
    ...
    public Cacao submitEntryIntoTheDatabase(){
        Cacao crab = null;
        // if(isEntrySufficient()) {
        if(false){
            crab = userInputToCacao();
            ContentValues contentValues = new ContentValues();
            contentValues.put(DatabaseContract.Cacao.COLUMN_FARM, etFarm.getText().toString());
            contentValues.put(DatabaseContract.Cacao.COLUMN_CITY, etCity.getText().toString());
            contentValues.put(DatabaseContract.Cacao.COLUMN_LATITUDE, latitude);
            contentValues.put(DatabaseContract.Cacao.COLUMN_LONGITUDE, longitude);
            contentValues.put(DatabaseContract.Cacao.COLUMN_WEIGHT,
                    Double.parseDouble(etWeight.getText().toString()));

            try{
                uriNew = getContentResolver().insert(
                        DatabaseContract.Cacao.CONTENT_URI,
                        contentValues
                );

                long idNewCacao = Long.parseLong(uriNew.getPathSegments().get(1));
                crab.setId(idNewCacao);

                Snackbar.make(buttonSubmit, "Cacao successfully added to the local database.", Snackbar.LENGTH_SHORT).show();
            }catch (Exception ex){
...
//            finish();
    ...
    public Cacao userInputToCacao(){
        Cacao crab = new Cacao();
    ...
    public void submitCacaoToServer(Cacao crab){
        new SubmitCacaoToServerTask().execute(crab);
    }
...
//                Snackbar.make(buttonSubmit, "Cacao successfully added to the remtoe database.", Snackbar.LENGTH_SHORT).show();
//
//                finish();
//
...
//
//                long idNewCacao = Long.parseLong(uriNew.getPathSegments().get(1));
//                Intent intent = new Intent(getBaseContext(), ViewCacaoActivity.class);
//                intent.putExtra(ViewAlbumsActivity.EXTRA_ID, idNewCacao);
...
//        }
    ...
    public Cacao cursorToCacao(Cursor cursor){
        ...
        crab.setId(id);
//        crab.setTag(cursor.getString(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_TAG)));
        crab.setStatus(cursor.getString(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_STATUS)));
        crab.setSerialNumber(serialNumber);
        crab.setCity(cursor.getString(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_CITY)));
        crab.setFarm(cursor.getString(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_FARM)));
        crab.setLatitude(cursor.getDouble(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_LATITUDE)));
        crab.setLongitude(cursor.getDouble(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_LONGITUDE)));
        crab.setWeight(cursor.getDouble(cursor.getColumnIndex(DatabaseContract.Cacao.COLUMN_WEIGHT)));
