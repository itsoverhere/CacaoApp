...
public class ChoosePhotoActivity extends AppCompatActivity {
    ...
    final static int REQUEST_CHANGE_CACAO = 2;
    ...
    Button buttonSubmit;
    Button buttonChangeCacao;
    ImageView ivImage;
    TextView tvIdCacao;

    private long idChosenCacao = -1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonChangeCacao = (Button) findViewById(R.id.button_change_cacao);
        ivImage = (ImageView) findViewById(R.id.iv_image);
        tvIdCacao = (TextView) findViewById(R.id.tv_id_cacao);

        if(getIntent().hasExtra(ViewCacaoActivity.EXTRA_ID_CACAO)){
            idChosenCacao = getIntent().getExtras().getLong(ViewCacaoActivity.EXTRA_ID_CACAO);
            tvIdCacao.setText(String.valueOf(idChosenCacao));
        }
        ...
        buttonSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(isEntrySufficient()) {
                    ...
                    contentValues.put(DatabaseContract.CacaoUpdate.COLUMN_DATE, Calendar.getInstance().getTimeInMillis());
                    contentValues.put(DatabaseContract.CacaoUpdate.COLUMN_PATH, chosenPath);
//                    contentValues.put(DatabaseContract.CacaoUpdate.COLUMN_IDCACAO, idChosenCacao);

                    getContentResolver().insert(
                            DatabaseContract.CacaoUpdate.CONTENT_URI,
                            contentValues
                    );
...
//                         .setAction("Action", null)
        ...
        buttonChangeCacao.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getBaseContext(), ViewAlbumsActivity.class);
                intent.putExtra(EXTRA_FROM_CHOOSE_PHOTO, true);
                startActivityForResult(intent, REQUEST_CHANGE_CACAO);
            }
        });
    ...
    public boolean isEntrySufficient(){
        ...
        if(idChosenCacao == -1){
            result = false;
            error += "Please choose a cacao to associate this image to. \n";
        }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if(requestCode == REQUEST_PERMISSION_CAMERA_STORAGE
            ...
            Snackbar.make(tvIdCacao, "Please try taking a picture again.", Snackbar.LENGTH_SHORT).show();
        }
    }
...
//            uriChosen = data.getData();
            ...
            Bitmap bmp = decodeSampledBitmapFromPath(chosenPath, ivImage.getWidth(), ivImage.getHeight());
            ivImage.setImageBitmap(bmp);
        }else if(requestCode == REQUEST_CHANGE_CACAO && resultCode == RESULT_OK){
            idChosenCacao = data.getExtras().getLong(ViewAlbumsActivity.EXTRA_ID);
            tvIdCacao.setText(String.valueOf(idChosenCacao));
        }
    }
    ...
    // TODO Add CacaoUpdate to DB
    // TODO Save return id to local DB's SERVERIDCACAOUPDATE
}
